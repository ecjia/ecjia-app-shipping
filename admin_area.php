...
class admin_area extends ecjia_admin 
{
	...
	public function init() 
	{
		...
		$shipping_id     = intval($_GET['shipping']);
		$args = array('shipping' => $shipping_id);
		...
		$this->assign('code', trim($_GET['code']));
		$this->assign('areas',    $ship_areas_list);
		$this->assign('ur_here', __('配送区域'));
		$this->assign('action_link', array('href'=>RC_Uri::url('shipping/admin_area/add', array('shipping' => $shipping_id, 'code' => trim($_GET['code']))),
				'text' => RC_Lang::lang('new_area')));
		$this->assign('full_page', 1);
		$this->assign('shipping_id', $shipping_id);
		$this->assign('form_action', RC_Uri::url('shipping/admin_area/multi_remove',$args));
	...
	public function add() 
	{
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here($shipping_data['shipping_name'], RC_Uri::url('shipping/admin_area/init', array('shipping' => $shipping_id, 'code' => $code))));
		...
		$this->assign('action_link',      array('text' => $shipping_data['shipping_name'].'列表', 'href' => RC_Uri::url('shipping/admin_area/init', array('shipping' => $shipping_id, 'code' => $code ))));
		$this->assign('default_country',  ecjia::config('shop_country'));
		$this->assign('region_get_url',   RC_Uri::url('shipping/region/init'));
		$this->assign('form_action',      RC_Uri::url('shipping/admin_area/insert'));
	...
	public function insert() 
	{
		...
		if ($area_count > 0) {
			...
			$new_id = $this->db_shipping_area->insert($data);
			...
			$links[] = array('text' => RC_Lang::lang('back_list'), 'href'=>RC_Uri::url('shipping/admin_area/init', array('shipping' => $shipping_id)));
			$links[] = array('text' => RC_Lang::lang('add_continue'), 'href'=>RC_Uri::url('shipping/admin_area/add', array('shipping' => $shipping_id)));
			// 			$refresh_url = RC_Uri::url('shipping/admin_area/edit', array('id' =>$new_id, 'keywords'=>$_POST['keywords'] ));
			$refresh_url = RC_Uri::url('shipping/admin_area/edit', array('id' =>$new_id));
	...
	public function edit() 
	{
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here( $shipping_data['shipping_name'], RC_Uri::url('shipping/admin_area/init', array('shipping' => $shipping_data['shipping_id']) )));
		...
		$this->assign('action_link', array('text' => $shipping_data['shipping_name'].'列表', 'href' => RC_Uri::url('shipping/admin_area/init', array('shipping' => $shipping_data['shipping_id']))));
		$this->assign('form_action', 'update' );
		RC_Loader::load_app_func('common', 'goods' );
		$this->assign('countries', $this->db_region->get_regions() );
		$this->assign('default_country', 1 );
		$this->assign('region_get_url',  RC_Uri::url ( 'shipping/region/init' ) );//区域联动使用
		$this->assign('form_action',     RC_Uri::url( 'shipping/admin_area/update' ));
	...
	public function update() 
	{
		...
		$shipping_id = intval($_POST['shipping']);
		$shipping_area_name = trim($_POST['shipping_area_name']);
		$shipping_area_id = intval($_POST['id']);
		$ship_area_count = $this->db_shipping_area->where(array('shipping_id' => $shipping_id, 
											'shipping_area_name' => $shipping_area_name,
											'shipping_area_id' => array('neq' => $shipping_area_id)) )->count();
		
		if ($ship_area_count > 0) {
			$this->showmessage( RC_Lang::lang('repeat_area_name'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		else {
			$shipping_data = $this->db_shipping->field('shipping_code,shipping_name')->find( array( 'shipping_id' => $shipping_id) );
			...
			//$links[] = array('text' => RC_Lang::lang('back_list'), 'href'=> RC_Uri::url( 'shipping/admin_area/init', array( 'shipping' => $_POST['shipping'] )) );
			// 			$refresh_url = RC_Uri::url('shipping/admin_area/edit', array('id' =>$shipping_area_id , 'keywords'=>$_POST['keywords'] ));
			$refresh_url = RC_Uri::url('shipping/admin_area/edit', array('id' =>$shipping_area_id));
	...
	public function multi_remove() 
	{
		...
		if (!empty($ids)) {
			...
			$refresh_url = RC_Uri::url('shipping/admin_area/init', array('shipping' => intval($_GET['shipping']), 'keywords'=>$_GET['keywords'] ));
	...
	public function get_shipareas_list($args = array())
	{
		...
		$ex_where = array('shipping_id' => $args['shipping']);
		if ($filter['keywords']) {
