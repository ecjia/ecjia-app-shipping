				        	...
				        	<div class="row-fluid">
				                <div class="span9 chat_content" id="code_shipping_print" {if $shipping.print_model == 2}style="display:none"{/if}>
			                        <div class="chat_heading clearfix">
				  					...
				  					<div class="text-box">
				  						<span class="help-block">{t domain="shipping" escape=no url="{url path='shipping/admin/print_template_preview' args="shipping_id={$shipping.shipping_id}"}" name="点击预览"}温馨提示：请复制右侧区域变量进行自定义模板设置 ；若未进行设置系统将会默认插件中的快递单模板<a href="%1" target="_blank">%2</a>；{/t}</span>
