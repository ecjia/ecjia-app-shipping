		...
		<table id="general-table" class="w900">
			<tr>
				<td colspan="2" class="chk_radio">
					<span><strong>请选择模板的模式：</strong></span>
					<input type="radio" name="model" class="uni_style" id="model_1" value="1" {if $shipping.print_model == 1}checked="checked"{/if} onclick="javascript:ecjia.admin.shipTemplate.template_radio_click('1');"><span>代码模式</span>&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="radio" name="model" class="uni_style" id="model_2" value="2" {if $shipping.print_model == 2}checked="checked"{/if} onclick="javascript:ecjia.admin.shipTemplate.template_radio_click('2');"><span>所见即所得模式</span>
					<span class="help-block cl_both" {if $help_open}style="display:block" {else} style="display:none" {/if} id="noticeGoodsSN">选择“代码模式”可以切换到以前版本。建议您使用“所见即所得模式”。所有模式选择后，同样在打印模板中生效。</span>
				</td>
			</tr>
			<tr id="visual" {if $shipping.print_model == 1}style="display:none"{else} style="display:block" {/if}>
				<td colspan="2" style="display: inherit;">
					<!--Flash播放器 start-->
					{assign var=goods_desc_src value=RC_Uri::url('shipping/admin/print_index',"shipping={$shipping_id}")}
				...
				<form class="form-horizontal" method="post" name="templateForm_1" action="{$form_action}">
					...
					<fieldset>
										...
										<div class="foldable-list">
											<div class="accordion-group">
												<div class="accordion-heading">
													<a class="accordion-toggle acc-in" data-toggle="collapse" data-target="#telescopic1"><strong>{$lang.shipping_template_info_t}</strong></a>
												</div>
												<div class="accordion-body in collapse" id="telescopic1">
													<div class="accordion-inner">
														<div class="control-group " >
															<div style=" float: left;width:30%;margin-left:10px;">{$lang.shipping_template_info_l}
															</div>
															<div  style=" float: left;width:30%;margin-left:10px;">{$lang.shipping_template_info_c}
															</div>
															<div  style=" float: left;width:30%;margin-left:10px;">{$lang.shipping_template_info_r}
															</div>
														</div>
													</div>
												</div>
						...
						<div class="control-group" id="code_submit" {if $shipping.print_model == 2}style="display:none"{/if}>
							<div class="controls" >
								<button class="btn btn-gebo" type="button" id="save_template_1">{$lang.button_submit}</button>
