...
<div class="row-fluid edit-page">
	<div class="span12">
		<input type="hidden" id="region_warn" value="{$lang.js_languages.region_exists}" />
		...
		<form id="form-privilege" class="form-horizontal" name="theForm" action="{$form_action}" method="post" enctype="multipart/form-data">
			<fieldset>
				...
				<div class="control-group formSep">
					<label class="control-label">{$lang.shipping_area_name}：</label>
					<div class="controls">
				...
				<div class="control-group formSep">
					<label class="control-label">{$lang.fee_compute_mode}：</label>
					<div class="controls chk_radio">
						...
						name="fee_compute_mode" value="by_number" /><span>{$lang.fee_by_number}</span>
					</div>
				</div>
				<!-- {/if} -->
				...
				<div class="control-group formSep">
					<label class="control-label">{t}选择配送区域{/t}：</label>
						...
						<div class="ms-container ms-shipping span12" id="ms-custom-navigation">
							<div class="ms-selectable span3">
								...
								<ul class="ms-list nav-list-ready selCountry" data-url="{url path='shipping/region/init' args='target=selProvinces&type=1'}" data-next="selProvinces">
									...
									<li class="ms-elem-selectable" data-val="{$country.region_id}"><span>{$country.region_name|escape:html}</span><span class="edit-list"><a href="javascript:;">添加</a></span></li>
									<!-- {foreachelse} -->
									<li class="ms-elem-selectable" data-val="0"><span>{t}没有国家地区可选…{/t}</span></li>
									<!-- {/foreach} -->
								</ul>
							</div>
							...
							<div class="ms-selectable span3">
								...
								<ul class="ms-list nav-list-ready selProvinces" data-url="{url path='shipping/region/init' args='target=selCities&type=2'}" data-next="selCities">
									<li class="ms-elem-selectable" data-val="0"><span>{t}请先选择省份名称…{/t}</span></li>
								</ul>
							</div>
							...
							<div class="ms-selectable span3">
								...
								<ul class="ms-list nav-list-ready selCities" data-url="{url path='shipping/region/init' args='target=selDistricts&type=3'}" data-next="selDistricts">
									<li class="ms-elem-selectable" data-val="0"><span>{t}请先选择市/区名称…{/t}</span></li>
								</ul>
							</div>
							
							<div class="ms-selectable span3">
								<div class="search-header">
									<input class="span12" type="text" placeholder="搜索的县/乡名称" autocomplete="off" id="selDistricts" />
								</div>
								<ul class="ms-list nav-list-ready selDistricts">
									<li class="ms-elem-selectable" data-val="0"><span>{t}请先选择县/乡名称…{/t}</span></li>
								</ul>
							</div>
						</div>
					</div>
				...
				<div class="control-group">
					<div class="controls">
						<button class="btn btn-gebo form_submit_btn" type="submit">{$lang.button_submit}</button>
					</div>
				</div>
			</fieldset>
		</form>
